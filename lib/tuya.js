...
const switchOnOff = (deviceId, enable) => {
 ...
 * @returns {Promise}
 */
exports.switchOn = deviceId => switchOnOff(deviceId, true)
/**
 * Switch off the plug
 * @param {string} deviceId the device ID
 * @returns {Promise}
 */
exports.switchOff = deviceId => switchOnOff(deviceId, false)
 ...
 */
exports.getDeviceStatus = async deviceId => {
  const res = await query('GET', `/v1.0/devices/${deviceId}/status`)
  return (res.result.find(e => e.code === 'switch') || {}).value
}
